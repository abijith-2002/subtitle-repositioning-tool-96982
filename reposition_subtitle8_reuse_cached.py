...
# Global cache to reuse detection results between detect_text_* and reposition_*
# Keyed by normalized time window (start_sec, end_sec), value is dict from get_detections
DETECTION_CACHE: Dict[Tuple[float, float], Dict[str, Any]] = {}

def _norm_key(start_sec: float, end_sec: float) -> Tuple[float, float]:
    """Normalize seconds to a stable cache key to reduce float jitter."""
...
def get_position_for_segment(video_path, start_sec, end_sec, min_frames=3):
    """Run OCR on sampled frames to decide top/bottom. First checks cache; fallback computes."""
    ...
    if cached and isinstance(cached, dict) and cached.get("recommended position"):
        log.info(f"Using cached decision for segment {key}: {cached.get('recommended position')}")
    ...
    position = decide_subtitle_position(filtered_detections_per_frame, frame_height)
    return position

from concurrent.futures import ThreadPoolExecutor, as_completed

def reposition_srt(video_path, srt_path, output_ass_path, min_frames=3, max_workers=5):
    """Read SRT and output ASS lines using cached decisions computed by detect_text_srt (no extra OCR)."""
    ...
    def process_sub(sub):
        ...
        if cached and cached.get("recommended position"):
            ...
            # Fallback if detect_text_srt wasn't called
...
def reposition_ass(video_path, ass_path, output_ass_path , max_workers = 5):
    """Modify only alignment tags in ASS dialogue lines using cached results from detect_text_ass (no extra OCR)."""
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        future_to_idx = {executor.submit(process_sub,line): i for i,line in enumerate(output_ass_file)}
        for future in as_completed(future_to_idx):
            i = future_to_idx[future]
            try:
                result = future.result()
                log.info(f"original [{i}] : {output_ass_file[i]}")
                log.info(f"result  [{i}] : {result}")
                output_ass_file[i] = result
                log.info(f"completed[{i}]")
            except Exception as e:
                log.error(f"Error processing line '{output_ass_file[i]}', Error:{e}")
                traceback.print_exc()
...
def reposition_ssa(video_path, ssa_path, output_ssa_path , max_workers =5 ):
    """Modify only alignment tags in SSA dialogue lines using cached results from detect_text_ssa (no extra OCR)."""
    def process_sub(line: str):
        if line.startswith("Dialogue:"):
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in as_completed(future_as_idx):
            ...
            try:
                result = future.result()
                log.info(f"original[{i}] : {results[i]}")
                log.info(f"result  [{i}] : {result}")
                results[i] = result
            except Exception:
                log.error(f"Error processing line: {results[i]}")
...
def reposition_vtt(video_path, vtt_path, output_vtt_path , max_workers = 5):
    """For VTT: modify 'line:' cue position using cached results (no extra OCR)."""
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in as_completed(future_as_idx):
            ...
            try:
                ...
                log.error(f"Error processing line: {results[i]} ({e})")
...
#----------------------------------------------------printing out detections and populating cache---------------------------------------------------

def get_detections(video_path, start_sec, end_sec, min_frames=3):
    """Run OCR on sampled frames and return analysis + recommended position. Results are cached for reuse."""
    key = _norm_key(start_sec, end_sec)
...
def detect_text_srt(video_path, srt_path, min_frames=3, max_workers=5):
    """Compute detections for SRT cues and store in cache for reuse by reposition_srt."""
    subs = pysrt.open(srt_path)
...
def detect_text_ass(video_path, ass_path, max_workers = 5):
    """Compute detections for ASS dialogue lines and store in cache for reuse by reposition_ass."""
    def process_sub(line:str):
        if line.startswith("Dialogue:"):
            m = re.match(r"Dialogue: \d+,(.*?),(.*?),", line)
            if m:
...
def detect_text_ssa(video_path, ssa_path,  max_workers =5 ):
    """Compute detections for SSA dialogue lines and store in cache for reuse by reposition_ssa."""
    def process_sub(line: str):
        if line.startswith("Dialogue:"):
            m = re.match(r"Dialogue: Marked=\d+,(.*?),(.*?),", line)
            if m:
...
def detect_text_vtt(video_path, vtt_path, max_workers = 5):
    """Compute detections for VTT cue timing lines and store in cache for reuse by reposition_vtt."""
...
def process_subtitle(video_path, subtitle_path, max_workers = 12):
    """Orchestrate detection (to populate cache) followed by reposition using cached decisions."""
    start = time.time()
