...
# Ensure numpy.NaN alias exists
if not hasattr(np, "NaN"):
    np.NaN = np.nan

# ------------------------------------------------------------------------------
# Whisper model is loaded once per worker process
# ------------------------------------------------------------------------------

# Worker-global variables for Whisper model reuse
_WHISPER_MODEL = None
_WHISPER_MODEL_SIZE = None


def _init_whisper_worker(model_size: str):
    """
    Initializer for ProcessPoolExecutor workers to load the Whisper model only once per worker.

    Args:
        model_size (str): Whisper model size name (e.g., "base", "small", "medium").
    """
    global _WHISPER_MODEL, _WHISPER_MODEL_SIZE
    try:
        if _WHISPER_MODEL is None or _WHISPER_MODEL_SIZE != model_size:
            # Force CPU for multi-process stability unless explicitly changed.
            _WHISPER_MODEL = stable_whisper.load_model(model_size, device="cpu")
            _WHISPER_MODEL_SIZE = model_size
            print(f"[Worker {os.getpid()}] Loaded Whisper model: {model_size}", flush=True)
    except Exception as e:
        # Leave globals as None to allow controlled failure in worker task.
        print(f"[Worker {os.getpid()}] Failed to load Whisper model '{model_size}': {e}", flush=True)
        _WHISPER_MODEL = None
        _WHISPER_MODEL_SIZE = None


def extract_audio(video_path):
    """Extract audio from video using ffmpeg into a unique temp directory."""
...
def transcribe_single_chunk(args):
    """
    Transcribe a single audio chunk using a preloaded Whisper model.

    Args:
        args (tuple): (chunk_file, offset, model_size, task, language)

    Returns:
        list[dict]: Transcribed segments with adjusted timestamps.
    """
    try:
        chunk_file, offset, model_size, task, language = args

        # Use worker-global model loaded by initializer; if not available (e.g., main-process retry),
        # lazily load it here to ensure robustness.
        global _WHISPER_MODEL, _WHISPER_MODEL_SIZE
        if _WHISPER_MODEL is None or _WHISPER_MODEL_SIZE != model_size:
            _WHISPER_MODEL = stable_whisper.load_model(model_size, device="cpu")
            _WHISPER_MODEL_SIZE = model_size

        result = _WHISPER_MODEL.transcribe(chunk_file, task=task, language=language)
...
def transcribe_chunks_parallel(chunks, model_size="medium", task="transcribe", language=None):
    """
    Transcribe multiple audio chunks in parallel using a worker-preloaded Whisper model.

    Args:
        chunks (list[tuple[str, float]]): List of (chunk_file, offset) pairs.
        model_size (str): Whisper model size to load in each worker (e.g., "medium").
        task (str): "transcribe" or "translate".
        language (str|None): Language code to pass to Whisper.

    Returns:
        dict: { "segments": list[dict], "language": language }
    """
    ...
    # Prepare argument tuples so we can easily retry in main process if a worker fails.
    args_list = [(chunk_file, offset, model_size, task, language) for (chunk_file, offset) in chunks]
    ...
    with ProcessPoolExecutor(
        ...
        future_to_args = {executor.submit(transcribe_single_chunk, args): args for args in args_list}

        for i, future in enumerate(as_completed(future_to_args), start=1):
            ...
            try:
                ...
                try:
                    ...
                    segs = transcribe_single_chunk(args)
                    all_segments.extend(segs)
...
def generate_subtitles(video_path, subtitle_lang):
    ...
    # 2. Split audio into chunks and detect language from the first chunk
    ...
    if chunks:
        ...
        # Fallback: if there are no chunks, nothing to process.
...
def main():
    video_path = r"Advanced HEBREW Conversation With Subtitles.mp4"
    sub_format = "srt"
    final_segments = generate_subtitles(video_path, "he")
